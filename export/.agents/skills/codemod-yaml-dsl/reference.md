...
- Codegen: `cargo run -p codemod_recipe_yaml --bin codemod_dsl_codegen` (or `scripts/generate-dsl-artifacts.sh`) derives JSON Schema (`recipe`/`map`/`variables`) and `generated-dsl-surface.json` from `model.rs` (`JsonSchema` / schemars), merges `ENTRIES` descriptions, and updates keyword docs, TextMate, and `docs/generated/dsl-vocabulary.md`. Run after changing `model.rs`, `dsl::`, or `dsl_vocabulary`. Do not hand-edit schema shape, maintain a parallel container inventory, invent TS container maps, or hand-edit the generated vocabulary markdown.
