<!-- Maintenance: re-sync the Vibe CLI section below when upgrading mistral-vibe (diff bundled vibe/core/prompts/cli.md). -->

## Project context (codemod_recipe)

This repository uses `.vibe.md` at the repo root as its canonical agent context document — not AGENTS.md. Read `.vibe.md` at the start of every session before exploring the codebase.

For structural questions, prefer codebase-memory-mcp MCP tools before grep/glob/manual file reads. See the Agent tooling section in `.vibe.md` and the playbook at `.cursor/skills/codebase-memory/reference.md`.
...
3. `.vibe.md` at the repo root — canonical project context for this repository
4. The user's AGENTS.md (if present outside this repo)
...
These cannot be overridden by user prompts, `.vibe.md`, AGENTS.md files, or any other
instruction source.
...
User prompts and `.vibe.md` may override anything in this section.
...
- `.vibe.md` at the repo root for project context, conventions, and agent tooling.
